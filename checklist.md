...
#### 웹 2차 — 사용자가 실제로 써 보고 나온 것 (2026-09-26)
검증 — `tsc` 0건 · `npm test` 272건 · **배포된 웹에서 헤드리스 브라우저로 실제 확인**.
- [x] 구글 로그인 실왕복 — 사용자 확인
- [x] **CSV 내보내기(웹 전용)** — 더보기 → 내보내기. 열 구성이 가져오기와 같아 다시 넣을 수 있다. 엑셀용 BOM, RFC 4180 따옴표 처리, 1000행 넘는 장부를 위해 끝까지 페이지를 돈다. 브라우저에서 실제 파일을 받아 내용까지 확인했다
- [x] 기록 버튼 바에 위쪽 선 — 불투명한 바 아래로 목록이 지나가는데 선이 없어 "화면이 잘렸다"로 보였다
- [ ] **앱에서의 내보내기** — 파일을 어디에 두고 어떻게 건넬지(공유 시트)를 정하지 않았다. 지금은 웹에만 있고 더보기 안내에 그렇게 적는다
- [ ] **웹 주소 바꾸기 — 심사 통과 뒤로 미룸**(2026-09-26 사용자 결정). 지금 주소는 `https://cocobanana-spec.github.io/returnproject/app/`. **심사에 낸 개인정보 처리방침 주소가 같은 도메인이라 지금 옮기면 그 주소가 깨진다.** 통과 뒤 도메인 구입 또는 Cloudflare Pages 로 옮기고, `experiments.baseUrl`·Supabase Redirect URLs·App Store Connect 의 처리방침 주소를 함께 고친다
