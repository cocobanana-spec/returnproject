...
### 확인 창 — 웹은 Alert 을 못 쓴다
**`react-native-web` 의 `Alert.alert` 은 빈 함수다**(`dist/exports/Alert/index.js`). 웹 1차에서 확인 창에 기대던 18곳이 전부 조용히 아무 일도 하지 않았다 — 로그아웃 버튼이 죽은 것처럼 보이고 기록 저장이 오류도 토스트도 없이 멈췄다(2026-09-26 QA 중대).

`src/lib/confirm.ts` 하나로 모았다. 화면은 `notify()`·`confirmAction()`만 부르고 플랫폼을 모른다.
- 웹 — `window.alert` / `window.confirm`. 예·아니오뿐이라 **무엇을 하는 확인인지 본문에 적는다**("[확인]을 누르면 삭제 합니다"). 파괴적 동작은 "되돌릴 수 없습니다"가 반드시 붙는다.
- 앱 — 지금까지와 같은 `Alert.alert`. 취소가 먼저, 파괴적 동작은 빨간 글씨.

**세 갈래는 `window.confirm` 에 담을 수 없다.** 두 번 묻는 것은 더 나쁘므로 갈래마다 다르게 풀었다.
| 자리 | 앱 | 웹 |
|---|---|---|
| 기록 저장 "이미 있는 행사예요"(취소/새 행사로/기존에 추가) | Alert 세 갈래 | **화면 안 선택지**를 저장 버튼 위에 펼친다 |
| 사람 삭제(취소/합치기/삭제) | Alert 세 갈래 | 예·아니오. 합치기는 그 화면의 "합치기" 버튼으로 이미 갈 수 있어 본문으로 안내한다 |

### 아직 아닌 것
소셜 로그인 웹 실제 왕복(Supabase Redirect URLs 등록 필요), 가져오기 웹 전체 흐름(파일 선택 대화상자), 반응형 재설계.

**웹에서 날짜를 바꿀 수 없다.** `@react-native-community/datetimepicker` 가 웹에서 `null` 을 돌려주어 날짜 선택기가 뜨지 않는다. 기록·행사의 날짜가 기본값(오늘)에 묶인다. 2차에서 웹용 `<input type="date">` 경로를 붙여야 한다.
