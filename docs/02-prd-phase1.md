...
> 작성일 2026-09-14 · 작성 product-planner · 검토·보완 cto-orchestrator · **2026-09-15 클라우드 확정 반영**
> 상위 전제 — docs/01(핵심 가치 3개·로드맵), docs/04(Expo 확정, 클라우드 확정 → Supabase 추천, 소셜 로그인 필수). 데이터 스키마는 docs/03에 있다.
...
- **목적** — 경조사에서 "누구에게 얼마를 냈고, 누구에게 얼마를 받았는지"를 사람 단위로 기록하고, 사람별 수지(준 합계 대 받은 합계)를 즉시 보여 준다. 1단계는 개인 장부다. 소셜 로그인 후 사용하며 데이터는 클라우드에 저장되어 기기를 바꿔도 로그인만으로 복원된다.
...
| US-13 | 앱을 그만 쓰려는 사용자로서, 계정과 내 데이터를 앱 안에서 완전히 삭제하고 싶다. 그래서 내 경조사 기록이 서버에 남지 않는다. | P0 |
...
- 날짜 — 기본 오늘. 탭하면 날짜 피커.
- 부조 형태 — 기본 현금. 칩으로 현금·이체·화환/조화·선물·없음(참석만).
- "더 입력" 펼침 — 장소, 참석 여부, 공동 부조자(이름 자동완성 1명), 메모.
...
- 상단 고정 — 행사 제목, 누적 "N명 · 합계 M원", 측 세그먼트(측 라벨이 있을 때만).
- 입력 영역 — 이름(자동완성, 3.2와 동일 규칙) → 금액(프리셋 + 키패드) → 부조 형태 토글(현금/이체, 나머지는 "더 보기") → 관계 그룹 칩(새 사람일 때만 노출).
- 키보드 위 고정 버튼 — "저장 후 다음". 탭하면 저장하고 이름창을 비운 뒤 다시 포커스. 측·부조 형태·관계 그룹은 직전 값을 유지한다(명부는 같은 측·같은 그룹이 뭉쳐 있다).
- 방금 저장한 항목은 입력 영역 바로 아래 목록의 맨 위에 쌓인다. 탭하면 인라인 수정·삭제.
...
- 더보기 → 데이터 내보내기 → "파일로 내보내기". JSON 1개 파일을 생성해 OS 공유 시트로 넘긴다(파일 앱·드라이브·메일 등). 파일명 `ppurin-export-YYYYMMDD.json`. 포맷은 docs/03 §10.
- 가져오기는 없다. 기기 변경은 로그인으로 해결되므로(US-12) 가져오기의 용도가 사라졌다. 내보내기는 사용자가 자기 데이터를 손에 쥐고 있다는 안심과 개인정보 이동권을 위한 것이다.

### 3.12 계정 — MVP 포함 (P0)

- 로그인 — 첫 실행 시 로그인 화면(S00). Apple 로그인·Google 로그인 버튼 2개(Kakao는 사용자 확인 질문 12번). 로그인 성공 시 홈으로. 세션은 앱이 유지하며 매 실행 시 다시 묻지 않는다.
- 첫 실행 방식은 "로그인 먼저"가 기본 가정이다(익명 시작 후 계정 연결은 사용자 확인 질문 13번).
- 로그아웃 — 더보기 → 계정 → 로그아웃. 로컬 캐시를 비우고 S00으로.
- 계정 삭제 — 더보기 → 계정 → 계정 삭제. 다이얼로그에 "사람 N명·행사 M건·기록 K건이 함께 삭제되며 되돌릴 수 없습니다" 표시 → 확인 → 서버에서 계정과 데이터 삭제 → S00. 💡 App Store는 계정 생성이 있는 앱에 앱 내 계정 삭제를 요구한다. 클라우드 선택으로 새로 생긴 필수 기능이다.
...
하단 탭 4개. 홈 · 사람 · 행사 · 더보기. 통계는 홈 카드에서 진입하고 더보기에도 항목을 둔다. 💡 기획 판단 — 통계를 탭으로 올리지 않은 이유는 1차 세그먼트가 매일 보는 화면이 아니기 때문이다. 탭은 "기록·사람·행사" 세 축에 집중한다.

### 4.2 화면 목록

| ID | 화면 | 목적 | 주요 요소 | 진입 | 이탈 |
|---|---|---|---|---|---|
| S00 | 로그인 | 계정 진입 | 앱 이름·한 줄 소개, Apple 로그인, Google 로그인, 개인정보 처리방침 링크 | 앱 실행(세션 없음), 로그아웃·계정 삭제 후 | 로그인 성공 시 S01 |
| S01 | 홈 | 요약과 최근 기록, 기록 시작점 | 올해 준돈/받은돈 카드 2개, 다가오는 행사 최대 3개, 최근 기록 10건, FAB "+ 기록", 오프라인 배너(네트워크 없을 때) | 앱 실행(세션 있음), 탭 | S02, S10, S07, S11 |
| S02 | 준돈 빠른 기록 | 30초 기록 | 이름 자동완성, 종류 칩, 금액 프리셋, 날짜, 형태, "더 입력" 펼침 | S01 FAB, S04 "이 사람에게 기록" | 저장 후 닫힘(토스트), 취소 |
| S03 | 사람 목록 | 사람 찾기 | 검색창, 그룹 필터 칩, 정렬, 행마다 이름·라벨·차액 | 탭 | S04, S05 |
...
| S06 | 행사 목록 | 행사 찾기 | 세그먼트(전체·내 행사·남의 행사), 연도 구분 헤더, 행마다 제목·날짜·건수·합계 | 탭 | S07, S08 |
...
| S09 | 받은돈 연속 입력 | 대량 입력 | 상단 누적, 측 세그먼트, 입력 영역, "저장 후 다음", 방금 저장 목록 | S07, S08 저장 직후 | 완료/뒤로 |
| S10 | 기록 상세/편집 | 1건 수정 | 사람(변경 가능), 행사(변경 가능), 방향(행사에서 파생, 표시만), 금액(미확정 토글), 형태, 참석, 측, 공동 부조자, 답례 체크·메모, 메모, 삭제 | S01, S04, S07, S12 | 저장/삭제/취소 |
| S11 | 통계 | 기간별 집계 | 연도 세그먼트, 총계, 종류별·그룹별 막대, 사람별 상위, 형태별 건수 | S01 카드, S13 | 행 탭 시 S04/S12 |
| S12 | 기록 검색/필터 | 조건 조회 | 검색창, 필터 칩, 결과 목록, 하단 합계 | S13, S11 행 탭 | S10 |
| S13 | 더보기 | 설정과 도구 | 통계, 기록 검색, 데이터 내보내기, 계정, 앱 잠금(P2), 앱 정보·개인정보 처리방침 | 탭 | S11, S12, S14, S16 |
| S14 | 데이터 내보내기 | 파일 내보내기 | 내보내기 버튼, 마지막 내보낸 날짜 | S13 | 완료 |
| S16 | 계정 | 로그아웃·계정 삭제 | 로그인 수단·이메일 표시, 로그아웃, 계정 삭제(확인 다이얼로그 + "먼저 내보내기" 링크) | S13 | 로그아웃·삭제 시 S00 |
...
5. 날짜는 오늘로 이미 채워져 있다. 형태는 "현금"이 기본. 둘 다 그대로 둔다.
...
2. S09 진입. 상단에 "내 결혼식 · 0명 · 0원", 측 세그먼트 "신랑측 | 신부측"(기본 신랑측). 이름창 포커스.
3. "박영수" 입력. 자동완성 없음(처음 보는 사람). 첫 행 "'박영수' 새 사람으로 추가" 탭. 관계 그룹 칩이 나타나고 "직장" 탭.
4. 금액 프리셋 "5만" 탭. 형태 토글은 "현금"(기본).
5. 키보드 위 "저장 후 다음" 탭. 목록 맨 위에 "박영수 · 직장 · 5만원 · 현금 · 신랑측"이 쌓이고 상단이 "1명 · 50,000원". 이름창이 비고 다시 포커스. 측·형태·그룹은 "신랑측·현금·직장"으로 유지.
...
8. 봉투에 "김철수·이영희" 두 이름. 이름 "김철수" 입력·선택 후 "더 보기" 펼쳐 "공동 부조자"에 "이영희" 입력·선택. 금액 10만 → 저장. 목록에 "김철수 (+이영희) · 10만원". 두 사람 원장 모두에 이 기록이 보이고, 상단 합계에는 10만원 한 번만 잡힌다.
9. "영업1팀 일동" 봉투 30만원. 이름 "영업1팀 일동" → 새 사람 추가 → 개인/단체 토글에서 "단체" 선택 → 30만 → 저장.
10. 실수로 5만원을 50만원으로 저장. 목록 맨 위 항목 탭 → 인라인 편집 → 금액 수정 → 확인.
11. 신랑측 끝. 세그먼트 "신부측" 탭. 그룹 기본값은 "기타"로 초기화(측이 바뀌면 관계 맥락이 바뀐다), 형태는 유지.
...
| 부조 형태 | 형태는 현금·계좌이체·화환/조화·선물(물품)·없음 5개. 금액은 형태와 독립이다. 화환·선물은 금액을 알면 넣고 모르면 미확정(NULL)으로 둔다. 통계의 금액 합계는 형태를 가리지 않고 amount를 합산하되, 형태별 섹션에서 "화환 N건·선물 M건"으로 건수를 따로 보여 준다. "참석했는데 부조 안 함"은 형태 "없음" + 금액 0 + 참석 예. "참석 안 하고 부조만"은 참석 아니오 + 금액. "노무/시간(도움만 줌)"은 형태 "없음" + 참석 예 + 메모. 참석 여부는 예/아니오/미기록 3값이며 기본 미기록. |
...
| 내 행사에 여러 당사자(양가) | 이벤트에 측 라벨 2개(`side_a_label`, `side_b_label`). 기록에 측(`a`/`b`/없음). 연속 입력 화면에서 측 세그먼트로 전환하며 입력한다. 행사 상세에서 측별 합계·건수·형태별 집계를 나란히 표시. 측 라벨이 비어 있으면 측 UI 전체를 숨긴다. 상주 3인 이상 등 3측 이상은 지원하지 않고 메모로 대체(❓ 3번). |
...
| 사람 삭제 | 기록이 없으면 즉시 삭제. 기록이 있으면 다이얼로그 "기록 N건도 함께 삭제됩니다. 다른 사람과 병합하시겠어요?"에 [병합] [기록까지 삭제] [취소]. 삭제는 서버 함수 한 번 호출로 원자 처리된다(docs/03 §6). 그 사람의 기록은 함께 지워지고, 공동 부조자로 들어간 기록은 공동 부조자만 비워지며, 그 사람이 당사자였고 기록이 없어진 남의 행사는 함께 지워진다. 병합은 세 참조 축(기록의 대표자·공동 부조자, 행사의 당사자)을 대상 사람으로 옮긴다. 삭제는 물리 삭제이며 휴지통·복구는 없다. |
...
| Apple 로그인 "이메일 숨기기" | 릴레이 이메일이 들어온다. 앱은 이메일을 표시용으로만 쓰고 식별에 쓰지 않는다(식별은 사용자 id). |
| 계정 삭제 | 확인 다이얼로그에 삭제될 건수와 "먼저 내보내기" 링크. 삭제 후 S00. 같은 소셜 계정으로 다시 로그인하면 빈 장부로 시작한다. |

## 6. MVP에서 제외할 것과 이유

| 제외 항목 | 이유 |
|---|---|
| 부부 공동 장부(계정 간 공유) | 소유 축이 바뀌는 큰 변경. 1단계 필수 여부를 사용자 확인 질문 11번으로 묻는다 |
...
| 이메일·비밀번호 로그인, 전화번호 인증 | 소셜 로그인 2종으로 충분. 비밀번호 재설정 흐름을 만들지 않는다 |
...
- [ ] 내 행사에서 연속 입력 모드로 10건을 입력할 때 화면 이동 없이 "저장 후 다음"만으로 이어지고, 측·형태·그룹이 직전 값을 유지한다.
- [ ] 금액을 비우고 저장하면 "미확정"으로 저장되고, 행사·사람·통계 합계에서 제외되며 "미확정 N건"이 표시된다.
- [ ] 공동 부조 1건(10만원)이 두 사람의 원장에 각각 10만원 "공동"으로 보이고, 행사 합계와 연도 합계에는 10만원 한 번만 잡힌다.
- [ ] 측 라벨이 있는 내 행사 상세에서 신랑측·신부측 합계와 건수가 따로 표시되고 그 합이 전체와 같다.
...
- [ ] 계정 A로 만든 사람·행사·기록이 계정 B로 로그인했을 때 조회·수정·삭제 어느 경로로도 보이거나 바뀌지 않는다(RLS. 다른 사용자의 행사 id로 기록 INSERT를 시도하면 실패한다).
- [ ] 기기 A에서 기록 후 기기 B에서 같은 계정으로 로그인하면 사람·행사·기록과 사람별 수지가 동일하게 보인다.
- [ ] 계정 삭제 후 같은 소셜 계정으로 다시 로그인하면 빈 장부이며, 서버 테이블에 이전 `user_id`의 행이 0건이다.
- [ ] 로그인 상태에서 앱을 종료하고 다시 열면 로그인 화면 없이 홈이 뜬다.
...
- [ ] 답례 체크 시 날짜가 기록되고, 내 행사 상세의 진행률 "완료 N / 전체 M"이 갱신된다.

## 8. 사용자 확인 필요 질문

### 8.1 확정된 항목 (2026-09-15 사용자 답변)

| # | 질문 | 결정 |
|---|---|---|
| 1 | 모바일 프레임워크 | **Expo** (추천안 그대로) |
| 2 | 저장 방식 | **클라우드** (추천안 "로컬 우선"과 다름. 영향은 context-notes.md §3.2) |
| 3 | 출시 플랫폼 | **iOS·Android 동시** (추천안 그대로) |

### 8.2 답변 대기 항목

| # | 질문 | 왜 중요한가 | 선택지 (★ 추천) |
|---|---|---|---|
| 4 | 앱 잠금(생체 인증)을 첫 출시에 넣을까요? | 기록 마찰과 민감 데이터 보호 사이의 선택이다 | ★ 첫 출시 제외, 후속 P2 / 첫 출시 포함(설정에서 켜는 방식, 기본 꺼짐) |
| 5 | 부부 공동 부조 10만원을 두 사람 원장에 어떻게 보여 줄까요? | 사람별 수지의 의미가 정해진다. 나중에 바꾸면 기존 기록 해석이 달라진다 | ★ 두 사람 모두 전액 10만원 + "공동" 배지 / 5만원씩 분할 표시 / 대표자에게만 10만원 |
| 6 | 관계 그룹은 고정 6개(가족·친척·직장·친구·지인·기타)로 충분합니까? | 필터·통계 축이다. 사용자 정의를 허용하면 그룹 관리 화면이 하나 더 필요하다 | ★ 고정 6개 + 구분 라벨 자유 입력 / 사용자 정의 그룹 추가 허용 |
| 7 | 화환·선물처럼 현금이 아닌 부조의 금액을 수지 합계에 포함할까요? | "받은 합계"의 정의가 달라진다. 화환 10만원을 받은 돈으로 볼지 사용자마다 다르다 | ★ 금액을 적었으면 포함, 안 적으면 미확정으로 제외 / 현금·이체만 합산하고 나머지는 건수만 |
| 8 | 옮겨 넣을 기존 기록이 엑셀·CSV 형태로 있습니까? 규모는 얼마입니까? | 수백 건 이상이면 CSV 가져오기를 P1로 올려야 한다. 수십 건이면 손 입력으로 충분하다 | ★ 손 입력(수십 건, 연속 입력 모드로 처리) / CSV 가져오기 P1 승격(수백 건 이상) |
| 9 | 다가오는 행사 알림(하루 전 로컬 알림)이 첫 출시에 필요합니까? | 알림 권한 요청이 하나 늘고, 예정 행사 등록 습관이 없으면 쓸모가 없다 | ★ 첫 출시 제외, 홈 "다가오는 행사" 섹션으로 대체 / 포함(행사 등록 시 알림 토글) |
| 10 | 내 행사에서 "참석했지만 부조 안 함"을 기록하고 싶습니까? | 명부 입력 시 참석 여부 입력이 한 단계 늘어난다. 필요 없으면 참석 필드를 "더 보기" 안에 숨긴다 | ★ 필드는 두되 "더 보기" 안에 숨김 / 연속 입력 기본 화면에 노출 |

### 8.3 클라우드 확정으로 새로 생긴 질문

| # | 질문 | 왜 중요한가 | 선택지 (★ 추천) |
|---|---|---|---|
| 11 | 부부(가족)가 한 장부를 함께 쓰는 기능이 1단계에 꼭 필요합니까? | 클라우드를 고른 이유가 이것이라면 데이터의 소유 축을 "사용자"가 아니라 "장부"로 설계해야 하고, 착수 후에 바꾸면 RLS 정책과 서버 함수를 전부 다시 써야 한다(docs/03 §9.3). 착수 전에만 싸게 바꿀 수 있다 | ★ 아니오, 1단계는 개인 장부(기기 변경 대비가 목적) / 예, 1단계부터 필요(소유 축을 장부로 재설계 후 착수) |
| 12 | 로그인 수단에 Kakao를 넣을까요? | Apple·Google은 스토어 요건상 기본이다. Kakao는 한국 사용자에게 익숙하지만 Kakao 개발자 앱 등록·검수 절차가 따로 든다 | ★ 첫 출시는 Apple + Google, Kakao는 후속 / 첫 출시부터 Kakao 포함 |
| 13 | 첫 실행에서 로그인을 먼저 요구할까요, 로그인 없이 쓰다가 나중에 계정을 연결하게 할까요? | 익명 시작은 "식장 가는 길 30초 기록"의 첫 진입 마찰을 없애지만, 계정을 연결하기 전에 앱을 지우면 데이터가 사라져 클라우드를 고른 목적과 충돌한다 | ★ 로그인 먼저(한 번만, 소셜 버튼 1탭) / 익명으로 시작하고 "계정 연결" 안내를 반복 노출 |
| 14 | 백엔드는 Supabase로 확정할까요? | 이후 스키마·RLS·인증 구현 전부가 이 결정 위에 놓인다. 비교와 근거는 docs/04 §2 | ★ Supabase(사용자 경험 3건, SQL 집계, 2단계 동일 프로젝트) / Firebase / 자체 API + Postgres |

## 📋 CTO 보고 요약

- 1단계 MVP는 P0 6개(계정(로그인·계정 삭제), 이벤트 등록, 준돈 빠른 기록, 받은돈 연속 입력, 사람 원장·수지, 이름 검색)와 P1 6개(필터, 통계 분해, 답례 체크, 연락처 피커, 날짜 정밀도, 데이터 내보내기)로 구성하고 알림·앱 잠금은 P2로 미뤘다.
- 클라우드 확정으로 로그인 화면(S00)·계정 화면(S16)이 추가되고 백업 가져오기가 빠졌다. 오프라인은 읽기 캐시 + 수동 재시도로 한정했다. 새 확인 질문 4개(공동 장부, Kakao, 익명 시작, Supabase 확정)를 §8.3에 두었다.
