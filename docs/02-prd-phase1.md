...
**2026-09-25 개편.** 상단 탭 전체·준 돈·받은 돈으로 나누고 각 탭에 연도 필터·경조사 종류 필터·정렬(금액순·건수순)을 둔다. 홈 받은돈 탭이 평평한 목록이 되면서 **행사별 구분이 이 화면으로 옮겨왔다**(사용자가 "행사별로 구분하는건 통계 메뉴에서"라고 명시). 행사별 블록은 `event_totals` RPC 한 번으로 받고 최신순·금액순·건수순으로 정렬한다. 행을 누르면 행사 상세(S07)로 간다.
...
| S11 | 통계 | 기간별·방향별 집계 | **전체·준 돈·받은 돈 상단 탭**, 연도 필터·경조사 종류 필터·정렬(금액순·건수순), 총계와 차액, 종류별·관계별 막대, **행사별**(최신순·금액순·건수순 — `event_totals`), 사람별 상위(`person_stats_by_year`) | 하단 탭 | 행사 행 탭 시 S07, 사람 행 탭 시 S04 |
