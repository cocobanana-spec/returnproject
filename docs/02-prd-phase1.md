...
### 3.14 명부 가져오기(엑셀·CSV) — P1, OCR보다 먼저 (2026-09-25 추가)

배경 — 사용자가 빌드 7을 쓴 뒤 "내 경조사 입력에 하나하나 입력 말고 import 하든지 OCR 인식 기능이 있으면 좋겠다. 대부분 엑셀로 정리하고, 직접 손으로 쓰기 때문에 사진으로 import하게 OCR이면 좋겠다"고 했다. 질문 8(손 입력, CSV 가져오기 없음)을 실사용 뒤 뒤집은 것이다. **이번 라운드는 기획만이며 구현하지 않는다.**

- 목적 — 내 행사 1건의 명부(받은돈)를 파일 한 번으로 넣는다. 명부 330명을 8초씩 40분 걸려 넣는 시나리오 B를 몇 분으로 줄인다.
- 흐름 — 내 행사 상세(S07) "명부 가져오기" → ① 파일 선택(엑셀 `.xlsx`, CSV) → ② 열 매핑(이름·금액·관계·메모. 첫 행이 제목인지 토글. 앱이 열 이름으로 추정해 미리 채우고 사용자가 고친다) → ③ 미리보기(행마다 이름·금액·관계·메모와 문제 표시) → ④ 확인 → ⑤ 저장.
...
- 라이브러리(2026-09-25 확인) — 파일 선택은 `expo-document-picker`(SDK 57 번들 버전 ~57.0.2, 미설치). 파일 읽기는 `expo-file-system`(~57.0.7). 엑셀 파싱은 SheetJS `xlsx`(npm 0.18.5, 순수 JS라 React Native에서 base64로 읽어 `XLSX.read(data, { type: 'base64' })`로 처리 가능. CSV도 같은 라이브러리로 읽힌다. 한글 CSV는 EUC-KR로 저장된 경우가 있어 UTF-8 실패 시 안내가 필요하다). 네이티브 재빌드가 필요한 것은 `expo-document-picker`뿐이다. `expo-file-system`은 expo 코어가 이미 끌고 와 pod에 들어 있고(57.0.7, `package.json`에 직접 의존성으로만 추가하면 된다), `xlsx`는 순수 JS다. 우리는 로컬 네이티브 빌드(`ios/`)를 쓰므로 추가 비용은 pod 설치와 빌드 번호 증가 정도다. 구현 라운드에서 정할 것 — ① `xlsx`의 npm 0.18.5는 SheetJS가 더 이상 갱신하지 않는 판이라 알려진 취약점 경고가 있고, 고친 판은 SheetJS 자체 CDN에서 tarball로 받아야 한다(출처를 결정해 적는다). ② base64 읽기는 SDK 54부터 `expo-file-system/legacy`의 `readAsStringAsync`이고 새 API는 `new File(uri).base64()`다. ③ EUC-KR CSV는 `XLSX.read(data, { type: 'base64', codepage: 949 })`로 직접 읽을 수 있어 "실패 시 안내"보다 낫다(미설치라 동작 미확인).
