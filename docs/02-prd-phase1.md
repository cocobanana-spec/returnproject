...
> 상위 전제 — docs/01(핵심 가치 3개·로드맵), docs/04(Expo + expo-sqlite + Drizzle, 로컬 우선, 로그인 없음). 데이터 스키마는 docs/03에 있다.
...
| 4 | 사람 삭제 시 기록 처리 | 경고 후 기록까지 함께 소프트 삭제, 같은 다이얼로그에서 병합 제안 **승인** | 사람 삭제의 대부분은 오타·중복 정리이며 그 해법은 병합이다. 차단만 하면 사용자가 기록을 일일이 지워야 한다 |
| 5 | 대리 부조(A가 B 돈을 대신 전달) | 스키마 없이 메모 처리, 기록의 사람은 돈의 주인 B **승인** | 수지 계산에 영향이 없고 조회 요구도 없다. 필요가 증명되면 컬럼 추가는 비파괴적이다 |

## 1. 개요

- **목적** — 경조사에서 "누구에게 얼마를 냈고, 누구에게 얼마를 받았는지"를 사람 단위로 기록하고, 사람별 수지(준 합계 대 받은 합계)를 즉시 보여 준다. 1단계는 개인 장부이며 서버·로그인 없이 기기 안에서만 동작한다.
...
| US-12 | 기기를 바꾸는 사용자로서, 장부 전체를 파일로 내보내고 새 기기에서 가져오고 싶다. 그래서 수년치 기록이 사라지지 않는다. | P0 |
...
- 근거 — 기록은 반드시 이벤트에 속한다(docs/04 CTO 결정 4). 이벤트가 있어야 "내 행사 정산"과 2단계 청첩장 연결이 가능하다.
...
- 범위 — 사람 생성/편집 화면의 "연락처에서 가져오기" 버튼. 시스템 연락처 피커에서 1명을 고르면 이름·전화번호를 채우고 연락처 식별자를 저장한다.
...
- 내보내기 — 더보기 → 백업 → "파일로 내보내기". JSON 1개 파일을 생성해 OS 공유 시트로 넘긴다(파일 앱·드라이브·메일 등). 파일명 `ppurin-backup-YYYYMMDD.json`.
- 가져오기 — 파일 선택 → 미리보기("사람 120명 · 행사 45건 · 기록 380건 · 내보낸 날짜") → 방식 선택. "병합"(같은 id는 updated_at이 최신인 쪽 유지, 없는 것은 추가)과 "전체 교체"(현재 데이터를 지우고 파일로 대체. 2단계 확인 다이얼로그).
- 포맷은 docs/03 §8.
- 근거 — docs/04 §2 CTO 결정. OS 자동 백업이 실패하는 경우(OS 간 기기 변경)의 최후 수단.
...
|---|---|---|---|---|---|
| S01 | 홈 | 요약과 최근 기록, 기록 시작점 | 올해 준돈/받은돈 카드 2개, 다가오는 행사 최대 3개, 최근 기록 10건, FAB "+ 기록" | 앱 실행, 탭 | S02, S10, S07, S11 |
...
| S13 | 더보기 | 설정과 도구 | 통계, 기록 검색, 백업, 앱 잠금(P2), 앱 정보·개인정보 처리방침 | 탭 | S11, S12, S14 |
| S14 | 백업 | 내보내기/가져오기 | 내보내기 버튼, 가져오기 버튼, 마지막 내보낸 날짜, 가져오기 미리보기·방식 선택 | S13 | 완료 |
| S15 | 동명이인 선택·병합 시트 | 중복 처리 | 후보 목록(이름·그룹·라벨·최근 기록·기록 수), "새 사람으로 추가" / 병합 대상 선택 | S02·S09 자동완성, S04 메뉴 | 선택/취소 |

첫 실행 온보딩 화면은 두지 않는다. 각 목록의 빈 상태에 한 줄 설명과 시작 버튼(예 "첫 기록을 남겨 보세요 → + 기록")을 넣는다.

### 4.3 화면 흐름도

```mermaid
flowchart LR
    S01[홈] -->|FAB| S02[준돈 빠른 기록]
    S02 -->|저장| S01
...
| 케이스 | 처리 방침 |
|---|---|
| 동명이인 | 사람은 UUID로만 구분하며 이름에 UNIQUE가 없다. 구분은 사용자가 붙이는 "구분 라벨"(예 "회사 동기", "고등학교")과 관계 그룹으로 한다. 자동완성 목록은 이름이 같은 후보를 모두 보여 주고 라벨·그룹·최근 기록 한 줄을 붙여 구별시킨다. 라벨이 둘 다 비어 있으면 저장 시점에 "같은 이름의 사람이 이미 있습니다. 라벨을 붙이시겠어요?"를 한 번 묻는다(건너뛰기 가능). 잘못 나뉜 경우는 S04 "병합"으로 합친다. 병합은 기록 전부를 대상 사람으로 옮기고 원본을 소프트 삭제하며, 되돌리기는 지원하지 않는다(확인 다이얼로그에 기록 수 표시). |
...
| 사람 삭제 | 기록이 없으면 즉시 소프트 삭제. 기록이 있으면 다이얼로그 "기록 N건도 함께 삭제됩니다. 다른 사람과 병합하시겠어요?"에 [병합] [기록까지 삭제] [취소]. 삭제 시 그 사람의 기록 전부 소프트 삭제, 그 사람이 공동 부조자로 들어간 기록은 `co_person_id`만 비운다. 그 사람이 당사자인 남의 행사는 살아 있는 기록이 없으면 함께 소프트 삭제하고, 다른 기록이 남아 있으면 당사자만 비운다. 병합은 사람을 참조하는 세 축(기록의 대표자·공동 부조자, 행사의 당사자)을 모두 대상 사람으로 옮긴다. 소프트 삭제된 행은 UI에 나타나지 않으며 1단계에는 휴지통·복구 화면이 없다(소프트 삭제는 2단계 업로드용이다). |
| 이벤트 삭제 | 다이얼로그 "기록 N건도 함께 삭제됩니다" → 이벤트와 기록 소프트 삭제. 사람은 남는다. 기록만 남기고 이벤트를 지우는 경로는 없다(기록은 반드시 이벤트에 속한다). |
| 빈 상태·초기 상태 | 모든 목록에 한 줄 안내와 시작 버튼. 홈 요약 카드는 0원 표시. |
| 가져오기 충돌 | 병합 시 같은 id는 `updated_at`이 최신인 쪽. 파일이 상위 포맷 버전이면 "앱을 업데이트하세요"로 거부. |

## 6. MVP에서 제외할 것과 이유

| 제외 항목 | 이유 |
|---|---|
| 서버·계정·동기화·부부 공동 장부 | docs/04 CTO 결정. 2단계에서 1회 업로드로 전환 |
| 청첩장 생성·공유·RSVP·축의금 계좌 안내 | 2단계 범위. 1단계는 `is_mine`과 UUID로 확장 문을 열어 둔다 |
...
| 휴지통·복구 | 소프트 삭제는 업로드용. 복구 UI는 필요가 증명되면 추가 |
| 외화·소수 금액 | docs/04 CTO 결정 2 |
...
- [ ] 연락처 권한을 거부해도 사람 생성·기록·통계·백업이 모두 동작한다.
- [ ] JSON 내보내기 → 앱 데이터 전체 삭제 → 가져오기(전체 교체) 후 사람·행사·기록 수와 사람별 수지가 내보내기 전과 같다.
- [ ] 가져오기 "병합"에서 같은 id의 기록은 updated_at이 최신인 쪽만 남는다.
...
| # | 질문 | 왜 중요한가 | 선택지 (★ 추천) |
|---|---|---|---|
| 1 | 모바일 프레임워크는 Expo로 확정할까요? | 이후 모든 구현 결정의 뿌리다. 2단계 웹 청첩장과 타입 공유 여부가 갈린다 | ★ A. React Native + Expo(TypeScript) / C. 네이티브 Swift 우선(iOS만 먼저 낼 때) |
| 2 | 저장 방식은 로컬 우선(로그인 없음)으로 갈까요? | 온보딩 마찰, 개인정보 책임, 2단계 전환 경로가 결정된다 | ★ 로컬 SQLite, 2단계에 1회 업로드 / 클라우드 우선(출시부터 부부 공동 관리가 필수일 때만) |
| 3 | 출시 플랫폼은 어느 쪽입니까? | 테스트 범위와 스토어 준비(개인정보 처리방침, 클로즈드 테스트 요건)가 달라진다 | ★ iOS·Android 동시 / iOS 먼저, Android는 이후 |
...
- 1단계 MVP는 P0 6개(이벤트 등록, 준돈 빠른 기록, 받은돈 연속 입력, 사람 원장·수지, 이름 검색, JSON 백업)와 P1 5개(필터, 통계 분해, 답례 체크, 연락처 피커, 날짜 정밀도)로 구성하고 알림·앱 잠금은 P2로 미뤘다.
