...
- 미리보기에서 표시하는 문제 — **동명이인**(이 장부에 같은 이름이 있거나 파일 안에 같은 이름이 둘 이상), **금액 형식 오류**(아래 규칙으로 못 읽는 값), **빈 이름**. 문제 행은 건너뛰거나 그 자리에서 고친다. 동명이인 규칙은 S02와 같다 — 라벨 없는 동명이인이 대량으로 생기는 자리이므로, 같은 이름이 있는 행은 구분 라벨을 넣기 전에는 저장하지 않는다(파일에 "구분" 열이 있으면 매핑으로 받는다).
...
| 케이스 | 처리 방침 |
|---|---|
| 동명이인 | 사람은 UUID로만 구분하며 이름에 UNIQUE가 없다. 구분은 사용자가 붙이는 "구분 라벨"(예 "회사", "고등학교")과 관계 그룹으로 한다. **2026-09-25 개정(사용자가 빌드 6에서 막힘).** ① 이름을 보여 주는 모든 곳(홈 두 탭·원장·자동완성·검색·통계 사람별·행사 상세)에서 라벨이 있으면 이름 뒤에 붙인다("김철수 · 회사"). 도메인 함수 `displayName` 하나로 통일한다. ② 자동완성·S15의 동명이인 행은 라벨 → 관계 그룹 → 주고받은 요약 → (라벨 없으면) 최근 기록 시점 순으로 구별 정보를 붙인다(`distinguishLine`). 데이터로 구별할 수 없으면 "구분 없음"을 적는다. ③ S02·S09에서 입력한 이름과 정규화가 같은 사람이 이미 있고 아직 기존 사람을 고르지 않았으면 그 자리에 구분 칸이 나타나고, 새 사람으로 저장할 때는 필수다("새 사람으로 추가" 행을 누르지 않고도 새 사람으로 저장할 수 있으므로 이름을 치는 즉시 보여 준다). 저장 직전에 서버에 같은 이름을 한 번 더 물어 자동완성이 실패했거나 8건 상한에 밀린 경우도 막는다. 같은 이름이 없으면 칸이 없다(다섯 필드 화면 유지). ④ 라벨 없이 이미 저장된 동명이인은 사람 목록(더보기)에서 행에 "구분 없음"을 표시해 편집(S05)으로 유도한다. 자동 병합·자동 라벨은 하지 않는다. 잘못 나뉜 경우는 S04 "병합"으로 합친다. 병합은 기록 전부를 대상 사람으로 옮기고 원본을 삭제하며, 되돌리기는 지원하지 않는다(확인 다이얼로그에 기록 수 표시). |
