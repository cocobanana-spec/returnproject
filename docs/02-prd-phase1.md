...
> 상위 전제 — docs/01(핵심 가치 3개·로드맵), docs/04(Expo·클라우드·Supabase 확정, 로그인 Apple+Google+Kakao). 데이터 스키마는 docs/03(소유 축 = 장부)에 있다.
...
- 로그인 — 첫 실행 시 로그인 화면(S00). Apple·Google·Kakao 로그인 버튼 3개(확정). 로그인 성공 시 서버가 개인 장부를 자동으로 만들어 두므로 앱은 바로 홈으로 간다. 세션은 앱이 유지하며 매 실행 시 다시 묻지 않는다. 로그인 없이 쓰는 경로는 없다(확정).
...
|---|---|---|---|---|---|
| S00 | 로그인 | 계정 진입 | 앱 이름·한 줄 소개, Apple 로그인, Google 로그인, Kakao 로그인, 개인정보 처리방침 링크 | 앱 실행(세션 없음), 로그아웃·계정 삭제 후 | 로그인 성공 시 S01 |
| S01 | 홈 | 요약과 최근 기록, 기록 시작점 | 상단에 현재 장부 이름, 올해 준돈/받은돈 카드 2개, 다가오는 행사 최대 3개, 최근 기록 10건, FAB "+ 기록", 오프라인 배너(네트워크 없을 때) | 앱 실행(세션 있음), 탭 | S02, S10, S07, S11 |
...
| S13 | 더보기 | 설정과 도구 | 장부, 통계, 기록 검색, 데이터 내보내기, 계정, 앱 잠금(P2), 앱 정보·개인정보 처리방침 | 탭 | S17, S11, S12, S14, S16 |
...
2. 아내 — 설치 → S00에서 Kakao 로그인 → 서버가 아내의 개인 장부(빈 "내 장부")를 만들고 홈으로. 빈 상태 안내에 "배우자가 이미 쓰고 있나요? 더보기 → 장부 → 초대 코드 입력" 한 줄.
...
| Apple 로그인 "이메일 숨기기", Kakao 이메일 미제공 | 릴레이 이메일이 들어오거나 이메일이 아예 없을 수 있다. 앱은 이메일을 식별에 쓰지 않는다(식별은 사용자 id). 구성원 표시 이름은 로그인 계정의 이름·닉네임, 없으면 이메일 앞부분, 그것도 없으면 "구성원". |
...
| 이메일·비밀번호 로그인, 전화번호 인증, 익명 시작 | 소셜 로그인 3종으로 충분. 비밀번호 재설정 흐름을 만들지 않는다. 익명 시작은 사용자가 배제했다 |
...
- [ ] 첫 로그인 직후 개인 장부 1권이 있고 사용자가 owner다. Apple·Google·Kakao 세 수단 모두로 확인한다.
...
### 8.1 확정된 항목 (2026-09-15 · 2026-09-16 사용자 답변)
...
| 11 | 부부(가족) 공동 장부 | **예, 1단계에 필요** (추천안 "아니오"와 다름. 소유 축을 장부로 재설계, docs/03) |
| 12 | 로그인 수단 | **Apple + Google + Kakao, 첫 출시부터** (추천안 "Kakao 후속"과 다름. Kakao 사실 확인은 docs/04 §3.1) |
| 13 | 첫 실행 | **로그인 먼저** (추천안 그대로. 익명 시작 없음) |
| 14 | 백엔드 | **Supabase 확정** (추천안 그대로) |
...
참고(질문 아님) — Kakao에서 이메일을 받으려면 비즈 앱 전환이 필요하다. 이 앱은 이메일을 식별에 쓰지 않으므로 전환 없이 진행 가능하며, 개인 개발자 전환 조건은 착수 시 Kakao Developers에서 확인한다(checklist.md).
