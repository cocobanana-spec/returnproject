...
- **받은돈의 경조사 종류 — 2026-09-26 개정(사용자가 빌드 15에서 막힘).** 받은돈은 행사에 속하고 종류는 행사가 가진다. 그래서 한 파일에 종류가 섞여 있으면 행사 하나에 다 넣을 수 없다.
  - **행사가 정해지지 않고 들어온 경우**(홈 받은돈 → 가져오기) — 파일의 구분 열을 종류로 쓰고 **종류별로 내 행사에 나눠 담는다.** 미리보기에 "장례식 12건 → 내 장례식 (새로 만듦)"처럼 묶음과 대상을 보여 주고 사용자가 바꿀 수 있다. 같은 종류의 내 행사가 있으면 그것이 기본값(여럿이면 가장 최근 것)이고, 없으면 새로 만든다. 새 행사의 날짜는 그 종류 행들의 가장 이른 날짜다.
...
| S16 | 장부 초기화 | 현재 장부 비우기 | 지워질 실제 건수(사람 N·행사 M·기록 K), **장부 이름을 그대로 입력해야 켜지는** 전부 지우기 버튼, 확인 다이얼로그. RPC `reset_ledger`(0007) | 더보기 → 장부 | 초기화 후 홈 |
