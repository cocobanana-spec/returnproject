...
- 이름 — 한 글자 이상 입력하면 이름 앞부분 일치(공백 제거·소문자 비교) 최대 8명 목록. 각 행에 표시 이름("이름 · 라벨")과 구별 줄(라벨 → 관계 그룹 → 주고받은 요약 → 최근 기록 시점)을 표시. 일치가 없으면 첫 행에 "'김철수' 새 사람으로 추가".
- 행사 종류 — 칩 6개. 기본 선택 없음(잘못 기록되는 것보다 한 번 더 탭이 낫다).
- 금액 — 프리셋 칩 5·10·20·30·50만원 + "직접 입력"(숫자 키패드, 만원 단위 토글). 프리셋을 탭하면 즉시 확정.
- 날짜 — 기본 오늘. 탭하면 날짜 피커.
- 메모 — 한 줄 입력. 항상 보인다.
- **2026-09-25 —** 입력 전 "최근 기록한 사람" 칩을 뺐다(사용자 요청, 필요 없다고 했다). 입력에 따른 자동완성은 그대로다. 화면 **오른쪽 위에 가져오기 버튼**이 있고 §3.14로 간다(대상은 준돈으로 정해진다).
...
- **2026-09-26 —** 행사 선택 줄은 **내 행사가 하나뿐이어도 보인다.** 그 줄에 "+ 새 행사 만들기"가 있어 돌잔치·장례식을 그 자리에서 만들 수 있다(만들면 명부 입력으로 돌아온다). 전에는 둘 이상일 때만 줄이 떠서, 결혼식 하나만 있는 사용자는 다른 종류를 만들 길이 아예 없었다(사용자 피드백 — "그냥 내 결혼식으로만 지정된다").
- 진입 — 내 행사 상세 → "명부 입력". 전체 화면 모드. **2026-09-25 —** 홈 받은돈 탭의 "+ 기록"도 이 화면으로 온다. 그때는 행사가 정해지지 않았으므로 **맨 위에서 내 행사를 고른다**(하나면 자동, 여럿이면 날짜가 가장 최근인 것이 기본값, 하나도 없으면 행사 만들기로 보낸다). 화면을 따로 만들지 않고 같은 저장 경로·같은 구분 라벨 규칙을 쓴다. 오른쪽 위에 가져오기 버튼이 있다.
...
**2026-09-25 개편.** 상단 탭 전체·준 돈·받은 돈으로 나눈다.

**2026-09-25 2차 — 가독성 정리(빌드 10 피드백 "가독성이 너무 떨어진다").** 기능을 더하지 않고 덜어냈다. ① 항상 보이는 조작은 **연도 칩 한 줄뿐**이고 경조사 종류 필터는 접어 둔다(기본값 "전체"로 대부분 충분하다). ② **막대 정렬 칩과 행사별 정렬 칩을 없앴다** — 막대는 금액 내림차순, 행사는 최신순으로 고정한다. 정렬 축을 고르게 하면 막대 길이 기준과 순서가 어긋나는 문제도 따라온다. ③ **'전체' 탭에서 막대 블록 4개를 뺐다.** 막대는 방향이 정해져야 뜻이 있으므로 준돈·받은돈 탭으로 옮겼다. 전체 탭은 총계·행사별·사람별만 보여 준다. ④ 숫자는 고정폭(tabular-nums)으로 건수 열·금액 열을 맞춰 세로로 훑을 수 있게 했다. ⑤ 사람별 상위는 "준 N · 받은 M" 두 숫자 대신 **차액 하나**만 보여 준다. 사람별 상위는 선택한 연도로 걸러 받으므로 안내 문구도 그 연도를 말한다(전에는 늘 "전체 기간"이라고 적어 사실과 달랐다). 금액 칸은 고정폭이 아니라 최소폭이고, 10억대가 들어와도 한 줄을 지킨 채 글자만 줄어든다. 홈 받은돈 탭이 평평한 목록이 되면서 **행사별 구분이 이 화면으로 옮겨왔다**(사용자가 "행사별로 구분하는건 통계 메뉴에서"라고 명시). 행사별 블록은 `event_totals` RPC 한 번으로 받고 최신순으로 고정 정렬한다. 행을 누르면 행사 상세(S07)로 간다.
...
### 3.14 가져오기(엑셀·CSV) — 2026-09-25 구현 완료

배경 — 사용자가 빌드 7을 쓴 뒤 "내 경조사 입력에 하나하나 입력 말고 import 하든지 OCR 인식 기능이 있으면 좋겠다. 대부분 엑셀로 정리하고, 직접 손으로 쓰기 때문에 사진으로 import하게 OCR이면 좋겠다"고 했다. 질문 8(손 입력, CSV 가져오기 없음)을 실사용 뒤 뒤집은 것이다.

**견본 열 구성(2026-09-25 사용자 제공).** 첫 행이 제목이고 `No | 이름 | 금액 | 구분`이다. 선행 열(A)이 비어 있다. 금액은 `₩100,000` 통화 서식이고, **구분 열은 관계 그룹이 아니라 경조사 종류**(`결혼식`)다. 날짜·메모·측 열은 없다.

**대상이 둘이라 첫 화면에서 고른다.** 견본처럼 구분 열에 종류가 있으면 보통 준돈 장부이지만, 같은 모양의 명부(받은돈)도 있다.
- **준돈 가져오기** — 행마다 그 사람을 당사자로 하는 남의 행사를 만들거나, 같은 사람·같은 종류·날짜 ±7일이면 기존 행사에 붙인다(S02 규칙). 날짜 열이 없으면 화면에서 날짜 하나를 받아 전체에 적용한다.
- **명부 가져오기** — 내 행사 하나를 고른 뒤 받은돈으로 넣는다. 구분 열은 무시하되 행사 종류와 다르면 미리보기에 경고만 띄운다.

- 목적 — 내 행사 1건의 명부(받은돈)를 파일 한 번으로 넣는다. 명부 330명을 8초씩 40분 걸려 넣는 시나리오 B를 몇 분으로 줄인다.
- 흐름 — ① 대상 선택 → ② 파일 선택(`.xlsx`, `.xls`, CSV) → ③ 열 매핑(첫 행이 제목인지 토글. 앱이 열 이름으로 추정해 미리 채우고 사용자가 고친다. `No|번호|순번`은 무시 열, 비어 있는 선행 열은 건너뛴다) → ④ 미리보기 → ⑤ 저장.
- 진입점 둘 — 내 행사 상세(S07)의 "명부 가져오기"(대상이 그 행사로 정해짐), 더보기 기록 관리의 "가져오기"(대상을 고름).
- 종류 매핑 — `결혼/결혼식/웨딩→wedding`, `돌/돌잔치→first_birthday`, `장례/장례식/조문/부고→funeral`, `회갑/칠순/팔순/환갑→senior_birthday`, `개업/개소→opening`, 그 밖은 `other`로 두고 미리보기에 표시한다.
- 미리보기에서 표시하는 문제 — **금액 형식 오류**(아래 규칙으로 못 읽는 값), **빈 이름**, **파일 안 중복**(같은 파일에 같은 이름이 둘 이상). 문제 행은 건너뛰거나 그 자리에서 고친다.
- **받은돈의 경조사 종류 — 2026-09-26 개정(사용자가 빌드 15에서 막힘).** 받은돈은 행사에 속하고 종류는 행사가 가진다. 그래서 한 파일에 종류가 섞여 있으면 행사 하나에 다 넣을 수 없다.
  - **진입로** — ① 기록 탭 받은돈의 가져오기 버튼, ② 더보기 → 가져오기 → 명부 가져오기. 두 길 모두 대상 선택에서 **"파일의 구분에 따라 나눠 넣기"가 기본 선택지**이고, 이때는 행사를 고르지 않아도 다음으로 간다(내 행사가 0건이어도 된다 — 전부 새로 만들어진다는 것을 미리보기가 보여 준다). "행사 하나에 넣기"는 두 번째 선택지다. ③ 행사 상세·명부 입력의 가져오기 버튼은 그 행사로 고정된다.
  - **나눠 넣기** — 파일의 구분 열을 종류로 쓰고 **종류별로 내 행사에 나눠 담는다.** 미리보기에 "장례식 12건 → 내 장례식 (새로 만듦)"처럼 묶음과 대상을 보여 주고 사용자가 바꿀 수 있다. 같은 종류의 내 행사가 있으면 그것이 기본값(여럿이면 가장 최근 것)이고, 없으면 새로 만든다. 새 행사의 날짜는 그 종류 행들의 가장 이른 날짜다.
  - **행사 상세(S07)에서 들어온 경우** — 대상이 고정되고, 구분 열이 그 행사의 종류와 다르면 경고만 한다(저장은 행사의 종류로 된다).
  - 판정은 `src/domain/importEvents.ts`가 한다. 화면은 묶음을 그리기만 한다.
- **같은 이름 처리 — 2026-09-26 개정(사용자가 빌드 11에서 막힘).** 사용자가 받은돈 명부를 가져오다가 "기존 장부에 같은 이름이 있다"는 수정 필요에 대량으로 걸렸다. **겹치는 것이 정상이고 대개 같은 사람이다.** 내가 축의금을 낸 김철수가 내 결혼식에 온 그 김철수다. 이 앱은 사람 하나에 준 돈과 받은 돈을 함께 보려고 만들었다(docs/01). 그래서 **기본값을 뒤집었다.**
  - **장부 후보가 정확히 1명** — 그 사람에게 **자동 연결**하고 통과시킨다. 조용히 붙이지 않는다. 미리보기 행에 `기존 "○○○"에 연결`이라고 적고, 칩으로 바꿀 수 있게 둔다.
  - **장부 후보가 2명 이상** — 그때만 진짜 모호하다. 누구인지 고르기 전에는 수정 필요다.
  - **사용자가 "다른 사람이에요"를 고른 경우** — 그때만 구분 라벨이 필수다.
  - **파일 안 중복** — 자동 연결하지 않는다. 그 둘이 같은 사람인지 모르는 채 둘 다 기존 한 사람에게 붙이면 남의 기록이 섞인다. "같은 사람"으로 정하면 그제야 장부 후보 1명에게 연결되고, "다른 사람"이면 **장부에 같은 이름이 없더라도** 각자 구분 라벨이 필요하다(둘을 나중에 가를 근거가 그것뿐이다). 짝을 건너뛰면 남은 한 행은 더 묻지 않는다.
  - **판정은 도메인 한 곳에서 한다.** 저장 가능 여부(`rowStatus`)와 구분 칸을 띄울 조건(`rowLabelNeeded`)이 같은 곳을 봐야 한다. 화면이 조건을 따로 조립하면 "저장은 막히는데 고칠 칸이 없는" 막다른 길이 생긴다(2026-09-26 QA에서 실제로 났다).
- 금액 표기 규칙 — 받는 것은 `100000`, `100,000`, `100000원`, `10만`, `10만원`, `10만 원`, `1만5천`, `15000.0`(정수인 소수)이다. `10`처럼 단위 없는 작은 수는 **원으로 읽되** 미리보기에서 "10원"으로 보여 사용자가 알아채게 한다(만원 단위로 추정하지 않는다. 추정이 틀리면 100배 오차가 조용히 들어간다). 빈 칸은 미확정(NULL). 음수·`-`·"화환"·"선물" 같은 문자는 형식 오류로 표시하고 금액을 비운다. 규칙은 순수 함수 `parseImportedAmount`로 두고 `node --test`로 고정한다.
- 저장 — 미리보기에서 확인한 행만 사람(새 사람이면 생성, 라벨 포함) → 기록 순차 INSERT. S09와 같은 2단 저장이며 중간 실패 시 "N행까지 저장됨"을 보여 주고 이어서 할 수 있게 한다. 형태는 `cash`, 측·공동 부조자는 넣지 않는다(입력 단순화와 같다).
- 파일은 앱 안에서만 읽는다. **서버에 파일을 올리지 않는다.** 서버로 가는 것은 사람·기록 행뿐이다.
- 라이브러리(2026-09-25 확정) — 파일 선택 `expo-document-picker ~57.0.2`(네이티브 재빌드 필요). 파일 읽기 `expo-file-system ~57.0.7`. 엑셀·CSV 파싱은 SheetJS `xlsx` **0.20.3을 SheetJS 공식 CDN tarball로 고정**했다. npm의 `xlsx` 최신판은 0.18.5에서 멈춰 있고 알려진 취약점 경고가 붙어 있어 쓰지 않는다. EUC-KR CSV는 `codepage: 949`로 직접 읽는다.
- 개인정보 처리방침 — 파일 접근은 시스템 문서 선택기를 통해 사용자가 고른 파일 1개만 읽으므로 별도 권한 항목은 없지만, "가져온 파일은 기기에서만 처리하고 서버에 저장하지 않는다"를 처리방침에 적는다.

### 3.14b 실제 사용자 파일에서 배운 것 (2026-09-26)

사용자가 자기 엑셀 화면을 보내 줬다. 열 구성이 `No | 성함 | 금액(원) | 일자 | 종류`였고, 날짜는 `11/15/22`, 종류는 `할아버지 장례식`이었다. **가정과 두 곳이 달랐고 둘 다 실패했다.**

- `금액(원)` — 열 이름 규칙이 `^금액$`으로 묶여 있어 괄호가 붙자 금액 열을 통째로 놓쳤다. 전 행이 `bad_amount`가 되어 저장이 막혔다. 이제 괄호부터 뒤를 설명으로 보고 떼어 낸 뒤 맞춘다.
- `11/15/22` — 미국식 월/일/연도를 못 읽어 날짜가 오늘로 들어갔다. `M/D/YY`·`M/D/YYYY`를 받고, 두 자리 연도는 2000년대로 읽는다. 13월이나 2월 30일 같은 없는 날짜는 버린다.
- `할아버지 장례식`은 이미 장례식으로 읽혔다. 종류 매핑이 부분 일치라 그렇다.
- `이민정(네이버)`처럼 괄호가 붙은 이름은 그대로 사람 이름이 된다. 회사를 구분에 적는 관습이라 일부러 떼지 않는다.

**교훈** — 파일 형식은 견본을 받기 전에는 알 수 없다. 견본 하나가 두 개의 치명적 실패를 드러냈다. 열 이름은 항상 꼬리가 붙는다고 보고, 날짜는 지역 형식이 섞인다고 본다.

### 3.14c 예시 표 (2026-09-26)

가져오기 파일 선택 화면에 예시 표를 보여 준다(`src/ui/SampleTable.tsx`). 이미지가 아니라 실제 표로 그린다. 이미지는 글자가 흐려지고 다크 모드에서 어색해지며 열 이름이 바뀌면 다시 만들어야 한다. 표의 값은 앱이 실제로 읽어 내는 모양 그대로이며, 금액 세 가지 표기와 날짜 세 가지 형식을 한 번에 보여 준다.

### 3.15 명부 사진 OCR — P1 후순위, 가져오기 미리보기 화면이 전제 (2026-09-25 추가)

- 목적 — 손으로 쓴 접수 명부를 사진으로 찍어 이름·금액을 뽑는다.
- **위험을 먼저 적는다. 손글씨 한글 인식은 정확도가 낮다.** 인쇄체는 괜찮지만 손글씨는 사람마다 편차가 커 이름 한 글자가 틀리면 다른 사람이 된다. 금액 자릿수가 틀리면 10배 오차가 난다. 그래서 인식 결과는 **절대 바로 저장하지 않고** 3.14의 미리보기·수정 화면을 그대로 거친다. 가져오기의 미리보기를 먼저 만드는 이유다.
- 후보 — ① iOS Vision 프레임워크 `VNRecognizeTextRequest`(온디바이스, 무료, 사진이 기기를 떠나지 않음. 한국어는 iOS 16부터 인식 언어에 포함되며 인쇄체 위주다. 손글씨는 편차가 크다). ② 클라우드 OCR(Google Cloud Vision·Naver CLOVA OCR 등. 손글씨 정확도가 더 높을 수 있으나 건당 비용과 **명부 사진이 외부로 전송되는 개인정보 문제**가 있다). **기본 방향은 온디바이스(①)** 이고, 정확도가 실사용에 못 미치면 그때 ②를 검토한다.
- Expo에서 Vision을 쓰는 비용 — Vision은 JS 모듈이 없어 **로컬 Expo 네이티브 모듈**(Swift, `expo-modules-core`)을 하나 만들어야 한다. 우리 iOS 배포 타깃은 16.4라 한국어 인식(iOS 16+)을 쓸 수 있다. 대안으로 iOS·Android를 한 번에 덮는 기성 ML Kit 패키지(예 `@react-native-ml-kit/text-recognition`, 한국어 스크립트 지원)가 있다. SDK 57 호환은 확인하지 못했으며 구현 라운드에서 Vision 직접 모듈과 비교해 고른다. 우리는 이미 로컬 네이티브 빌드와 설정 플러그인(`plugins/withIosSceneLifecycle.js`)을 쓰고 있어 가능하다. 대략 Swift 파일 1개(이미지 → 인식 문자열·좌표 배열)와 모듈 정의, Android는 ML Kit로 따로 구현해야 한다. 표에서 이름 열과 금액 열을 맞추는 후처리(좌표로 같은 줄 묶기)가 인식 자체보다 더 손이 간다.
- 사진 입력 — `expo-image-picker`(~57.0.19, 미설치)로 카메라·앨범 접근. **개인정보 처리방침에 카메라·사진 접근 권한 항목과 "사진은 기기에서만 처리한다(온디바이스일 때)"를 추가해야 한다.** Info.plist에 `NSCameraUsageDescription`·`NSPhotoLibraryUsageDescription`이 필요하다.
- 우선순위 — 3.14 가져오기 → (미리보기 화면 완성 뒤) 3.15 OCR.

사용자에게 물을 것
- 엑셀 열 구성이 보통 어떤지 — 견본 파일 1개를 받으면 열 매핑 추정과 금액 규칙을 그에 맞춘다(예 "이름 / 금액 / 관계 / 비고", 측 열이 있는지, 금액이 "10만"인지 "100,000"인지).
- OCR을 첫 출시에 넣을지 뒤로 미룰지 — 손글씨 정확도 위험을 알고도 첫 출시에 원하는지. 미루면 가져오기만 먼저 나간다.
- 손글씨 명부의 실제 사진 2~3장 — 온디바이스 인식률을 먼저 재 보고 클라우드 필요 여부를 정한다.
...
**2026-09-26 개정(사용자 요청).** 하단 탭 **4개. 홈 · 기록 · 통계 · 더보기.**

- **홈** 은 대시보드다. **준 돈 총액과 받은 돈 총액 두 숫자**를 크게 보여 준다. 전체 기간이다. 요청이 "딱 2개 항목"이었으므로 그 아래는 미확정 안내 한 줄과 기록 보기 버튼뿐이다.
- **기록(S01b)** 은 예전 홈을 그대로 옮긴 것이다. 아래 홈 설명은 이제 기록 탭의 설명이다.
- **마지막으로 보던 탭을 기억한다.** 앱을 다시 열면 그 탭으로 열리고, 저장된 값이 더 이상 없는 탭이면 홈이다. 로그아웃 때 지운다(다른 계정이 이전 사용자의 위치를 물려받지 않게).

**2026-09-24 개정 — 사용자가 실기기에서 써 본 뒤 요청한 구조다.** 하단 탭 3개. 홈 · 통계 · 더보기.

- **기록(S01b, 2026-09-26 전에는 홈)** 은 준돈·받은돈 **상단 탭 2개**로 나뉜다. 기본은 준돈이고 둘 다 행사 날짜 최신순 내림차순이다. 각 탭은 기록 목록이며 한 행에 사람·행사·날짜·금액이 보인다. **행의 사람 이름을 누르면 사람 원장(S04)으로 간다.** 공동 부조 "김철수 (+이영희)"는 각 이름이 각자의 원장으로 간다(2026-09-24 결정, 사용자가 반대하면 되돌린다). 사람 탭을 없앤 대신 이곳이 사람별 수지로 가는 주 진입로다.
- 기록 탭 우상단에 **검색 버튼**이 있고 사람 검색(S03b)을 연다.
- **통계(S11)** 가 탭으로 올라왔다. 💡 기획 판단 변경 — 애초에 "매일 보는 화면이 아니다"라고 탭에서 뺐지만, 사용자가 직접 써 보고 통계를 탭에 두기를 원했다. 실사용자의 요청이 사전 추정보다 우선한다.
- **사람 목록(S03)과 행사 목록(S06)은 탭에서 빠져 더보기(S13) 안으로 들어갔다. 화면은 지우지 않는다.** 사람 화면에는 병합·삭제 같은 관리 동작이 있고, 받은돈은 행사에 속해야만 기록되므로 행사 화면이 사라지면 명부 입력(S09)으로 가는 길이 끊긴다.
- 하단 "+ 기록" 버튼은 그대로 둔다. 사용자가 명시적으로 좋다고 한 부분이다. **2026-09-25 —** 이 버튼은 보고 있는 탭을 따른다. 준돈 탭은 S02, **받은돈 탭은 S09**로 간다(받은돈은 행사에 속하므로 준돈 화면으로 보내면 방향이 뒤집힌 기록이 저장된다 — 실제로 그 버그가 있었다).
- **2026-09-25 —** 홈 상단 요약은 **총액 한 줄**만 남겼다(연도·건수 표기 제거, 사용자 요청). 받은돈 탭의 행사로 가는 줄도 뺐다. 두 탭이 같은 평평한 목록이며 **행사별 구분은 통계(S11)에 있다.**
...
| S01 | 홈 | 방향별 기록 목록과 기록 시작점 | 상단에 현재 장부 이름과 검색 버튼, 준돈·받은돈 상단 탭(기본 준돈), 탭마다 **총액 한 줄**, 준돈 탭에는 다가오는 행사 띠 최대 3개, 최신순 기록 목록(무한 스크롤), FAB "+ 기록"(준돈 탭→S02, 받은돈 탭→S09), 오프라인 배너 | 앱 실행(세션 있음), 탭 | S02, S03b, S04, S10, S06, S07 |
| S02 | 준돈 빠른 기록 | 30초 기록 | 이름 자동완성, 날짜, 종류 칩, 금액 프리셋, 메모 — 다섯 필드뿐(2026-09-24). 같은 이름이 있을 때만 구분 칸(필수). 오른쪽 위 가져오기 버튼. 최근 기록한 사람 칩은 뺐다(2026-09-25) | S01 FAB, S04 "이 사람에게 기록" | 저장 후 닫힘(토스트), 취소 |
...
| S05 | 사람 생성/편집 | 정보 입력 | 이름, 그룹, 구분 라벨(동명이인 정리의 도착지 — 사람 목록의 "구분 없음"에서 온다), 전화, 메모, 개인/단체, "연락처에서 가져오기" | S03 +, S04, S02·S09의 새 사람 | 저장/취소 |
| S06 | 행사 목록 | 행사 찾기 | 세그먼트(전체·내 행사·남의 행사), 연도 구분 헤더, 행마다 제목·날짜·종류 배지 | S13 더보기 | S07, S08 |
...
| S09 | 받은돈 연속 입력 | 대량 입력 | (행사 없이 들어오면) 내 행사 선택, 상단 누적, 이름·금액·메모 입력, 같은 이름이 있을 때만 구분 칸 한 줄, "저장 후 다음", 방금 저장 목록, 오른쪽 위 가져오기 버튼 (측·형태·공동 부조자는 2026-09-24 입력에서 제외) | S07, S08 저장 직후 | 완료/뒤로 |
...
| S16 | 장부 초기화 | 현재 장부 비우기 | 지워질 실제 건수(사람 N·행사 M·기록 K), **장부 이름을 그대로 입력해야 켜지는** 전부 지우기 버튼, 확인 다이얼로그. RPC `reset_ledger`(0007, 0008에서 **owner만**으로 좁힘 — 구성원 제거도 owner만인데 그보다 파괴적인 동작이 더 느슨할 이유가 없다) | 더보기 → 장부 | 초기화 후 홈 |
| S11 | 통계 | 기간별·방향별 집계 | **전체·준 돈·받은 돈 상단 탭**, 연도 칩(항상 보임), 경조사 종류 필터(접힘), 총계와 차액, 종류별·관계별 막대(준돈·받은돈 탭에만), **행사별**(최신순 — `event_totals`), 사람별 상위(`person_stats_by_year`). 숫자는 고정폭으로 같은 열에 맞춘다 | 하단 탭 | 행사 행 탭 시 S07, 사람 행 탭 시 S04 |
| S12 | 기록 검색/필터 | 조건 조회 | 검색창, 필터 칩, 결과 목록, 하단 합계 | S13, S11 행 탭 | S10 |
| S01b | 기록 | 준돈·받은돈 목록 | 상단 탭(준 돈·받은 돈), 최신순 목록, 방향별 총액, 검색, 기록 FAB. **2026-09-26에 홈에서 이 탭으로 옮겨 왔다** | 하단 탭 | 이름 탭 시 S04, 행 탭 시 S08 |
...
| S15 | 동명이인 선택·병합 시트 | 중복 처리 | 후보 목록(표시 이름 "이름 · 라벨", 구별 줄 라벨→그룹→요약→최근 시점), "새 사람으로 추가" / 병합 대상 선택 | S02·S09 자동완성, S04 메뉴 | 선택/취소 |
...
2. "김ㅊ" 입력. 목록에 "김철수 · 직장 · 2024.05 결혼식 10만원 받음"과 "김철민 · 친구 · …"가 뜬다. 김철수 행 탭. 이름창이 "김철수" 칩으로 바뀌고 종류 칩으로 포커스 이동.
   - 분기 — 같은 이름이 2명 이상이면 목록에 둘 다 라벨과 최근 기록을 붙여 보여 준다. 사용자는 그중 하나를 고르거나 "새 사람으로 추가"를 택한다. 별도 화면 없이 목록 안에서 해결한다. S15 시트는 병합 갈래로만 남는다(2026-09-25, 동명이인 확인은 S02 인라인 구분 칸이 맡는다).
   - 분기 — 일치가 없으면 첫 행 "'김철수' 새 사람으로 추가" 탭. 관계 그룹 칩 6개가 이름창 아래 한 줄로 나타난다(선택 안 해도 저장 가능, 기본 "기타"). **같은 이름이 이미 있는데 "새 사람으로 추가"를 고르면** 그 자리에 구분 칸 "같은 이름이 이미 있어요. 구분할 말을 적어 주세요(예: 회사, 고등학교)"가 나타나고 이때는 필수다(2026-09-25).
3. 종류 칩 "결혼식" 탭.
...
| 케이스 | 처리 방침 |
|---|---|
| 동명이인 | 사람은 UUID로만 구분하며 이름에 UNIQUE가 없다. 구분은 사용자가 붙이는 "구분 라벨"(예 "회사", "고등학교")과 관계 그룹으로 한다. **2026-09-25 개정(사용자가 빌드 6에서 막힘).** ① 이름을 보여 주는 모든 곳(홈 두 탭·원장·자동완성·검색·통계 사람별·행사 상세)에서 라벨이 있으면 이름 뒤에 붙인다("김철수 · 회사"). 도메인 함수 `displayName` 하나로 통일한다. ② 자동완성·S15의 동명이인 행은 라벨 → 관계 그룹 → 주고받은 요약 → (라벨 없으면) 최근 기록 시점 순으로 구별 정보를 붙인다(`distinguishLine`). 데이터로 구별할 수 없으면 "구분 없음"을 적는다. ③ **2026-09-26 재개정.** 같은 이름이 있다는 것만으로는 아무것도 요구하지 않는다. **기본은 "이 사람이 그 사람이다"**이다. S02·S09에서 이름을 쳤을 때 같은 이름이 한 명 있으면 저장 시 그 사람에게 붙고, 구분 칸은 사용자가 **"새 사람으로 추가"를 누른 뒤에만** 나타난다. 같은 이름이 둘 이상이면 자동완성에서 누구인지 고르라고 한다. 저장 직전에 서버에 같은 이름을 한 번 더 물어 같은 규칙으로 판정한다(자동완성이 실패했거나 8건 상한에 밀린 경우를 막는다). 판정은 도메인 함수 `sameNameDecision`·`resolveSameName` 하나가 하고 S02·S09·가져오기가 함께 쓴다. 같은 이름이 없으면 칸이 없다(다섯 필드 화면 유지). ④ 라벨 없이 이미 저장된 동명이인은 사람 목록(더보기)에서 행에 "구분 없음"을 표시해 편집(S05)으로 유도한다. 자동 병합·자동 라벨은 하지 않는다. 잘못 나뉜 경우는 S04 "병합"으로 합친다. 병합은 기록 전부를 대상 사람으로 옮기고 원본을 삭제하며, 되돌리기는 지원하지 않는다(확인 다이얼로그에 기록 수 표시). |
...
| CSV/엑셀 가져오기 | 열 매핑 UI가 크다. 사용자 확인 질문 8번의 답에 따라 P1 승격 검토 **2026-09-25 — 사용자 실사용 뒤 §3.14로 P1 승격.** |
...
- [ ] 이름 자동완성이 "김ㅊ" 같은 부분 입력에 반응하고, 동명이인 2명이 라벨·그룹·최근 기록으로 구별돼 표시된다. 같은 이름 두 사람의 표시 문자열이 같으면 단위 테스트가 실패한다.
...
| 8 | 기존 기록을 CSV로 옮길 필요가 있습니까 | **손 입력. CSV 가져오기는 P1로 올리지 않는다** | 나중에 필요해지면 열 매핑 화면을 새로 만든다. **2026-09-25 — 사용자가 빌드 7 실사용 뒤 뒤집었다.** 대부분 엑셀로 정리하고 손으로 쓴 명부는 사진으로 넣고 싶어 한다. §3.14 가져오기(먼저)·§3.15 OCR(뒤)로 기획했고 이번 라운드는 문서만이다 |
